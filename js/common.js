function createInput(appendHere, divAddClass, id, label, placeholder, enterFunc=null, labelClass="") {
  let theInput = $(`<div class="form-inline w-100${divAddClass}"></div>`).append(`<label for="${id}" class="px-2 text-right${labelClass}">${label}</label>`).append(`<input type="text" class="form-control pcm_inputText-md ml-2 text-left" id="${id}" placeholder="${placeholder}">`).appendTo(appendHere);
...
function displayObjectData(thisArrayObject, divContainer, thisObject, table=false) {
...
function displayObjectData1(thisArrayObject, divContainer, thisObject, table=false, horizontal=false) {
  let row=null;
  if (horizontal) row = $(`<tr class=""></tr>`).appendTo(divContainer);
  thisArrayObject.forEach(element => {
    ...
    let pre = (element.pre) ? element.pre : "";
    let tdWidth = (element.width) ? `width:${element.width} !important;` : "";
    let tdStyle = `style="padding-right:1px !important; max-width:200px; ${tdWidth}"`;
    if (table & !horizontal) row = $(`<tr class="d-flex"></tr>`).append($(`<td class="col-4 text-right">${element.label}</td>`)).appendTo(divContainer);
    else if (!horizontal) row = $(`<div></div>`).append($(`<span class="${padding}">${element.label}</span>`)).appendTo(divContainer);
    if (table) valueCol = $(`<td class="font-weight-bold text-left px-1 py-1 text-pcmInfo text-truncate"${tdStyle}></td>`).appendTo(row);
    else valueCol = $(`<span class="font-weight-bold pl-2 text-left text-info"><span></span></span>`).data("edit","off").appendTo(row);
    if (element.type==="range") {
      $(`<input class="pcm_inputRange" type="range" min="${element.min}" max="${element.max}" value="${theValue}"></input>`).on('input', (e) => { $(`#pcm_${element.key}Detail`).val(($(e.target).val())); thisObject[element.key] = $(e.target).val(); } ).appendTo(valueCol);
      $(`<input class="pcm_inputRangeText" id="pcm_${element.key}Detail" type="text" value="${theValue}" size="2"></input>`).appendTo(valueCol);
      ...
      const button = $(`<button class="btn btn-primary${element.addClass}" id="pcm_collectButton1_${element.unique}">${element.label}</button>`);
      if (element.btnFunc) $(button).on('click', {unique:element.unique}, (e) => { element.btnFunc.apply(this, [e]) })
      $(button).appendTo(valueCol);
    } else if (element.type==="checkbox") {
        createCheckBox(valueCol, "", `pcm_selection_${element.unique}`, element.unique, "", " m-0", element.inputClass);
    } else if (element.type==="keyValue") {
        const id = (element.id) ? ` id=${element.id}` : ``;
        $(`<span${id}>${pre}${theValue}</span>`).appendTo(valueCol);
    } else if (element.type==="string") {
      const border = (element.noBorder) ? "" : " class='border'"
        $(`<span${border}>${element.string}</span>`).appendTo(valueCol);
    }
  });
}
