...
class ModalSearchClass {
  ...
  showTriggerAddModal(afterClose=null, doCustom=false) {
    ...
    let wrongInput = (doc, errorStr=null, theProblem=null) => {
      ...
      $(doc).find('.pcm-checkStatus.pcm-inputError:first').html((errorStr) ? errorStr : 'Must fill in GroupID, Requester ID or URL!').data('gIdEmpty',true);
    }
    /** Verifies that the groupID inputted is correct.
     * @async                 - To wait until trigger is added.
     * @param {bool} doCustom - Custom Trigger?  @param {object} doc - Document Fragment */
    let checkTrigger = async (doCustom, doc, data) => {
      let groupVal = $('#pcm-formAddGroupID').val(), trigName = $('#pcm-formTriggerName').val(), minPay = parseFloat($('#pcm-formMinPay').val());
      if (doCustom && groupVal.length <= 3) wrongInput(doc, 'All custom Triggers MUST have a word or phrase to search for with more than 3 characters!', $(`label[for='pcm-formAddGroupID']`));
      else if (doCustom && trigName.length <= 3) wrongInput(doc, 'You must fill in the Unique Trigger Name!', $(`label[for='pcm-formTriggerName']`));
      else if (doCustom && (isNaN(minPay) || minPay === 0)) wrongInput(doc, 'All custom Triggers need to have a minimum pay rate!', $(`label[for='pcm-formMinPay']`));
        ...
        if (!doCustom && !reqId && !groupId) wrongInput(doc, _, $(`label[for='pcm-formAddGroupID']`));
        else {
          ...
          else wrongInput(doc, 'There is already a trigger with this value. Sorry. Please try again.', $(`label[for='pcm-formAddGroupID']`));
          theRules = null;
        }
      } else wrongInput(doc, _, $(`label[for='pcm-formAddGroupID']`));
    }
    ...
    let idName = modal.prepareModal(null, '920px', 'pcm-addTriggersModal', 'modal-lg', 'Add new Search Trigger', '<h4>Enter New Search Trigger Information.</h4>', 'pcm-searchModal', '', 'visible btn-sm', 'Add new Search Trigger', () => { checkTrigger(doCustom, df, data); }, 'invisible', 'No', null, 'visible btn-sm', 'Cancel');
    modal.showModal(null, () => {
      let example1Text = 'example: 3SHL2XNU5XNTJYNO5JDRKKP26VU0PY', example2Text = 'example: Ibotta receipts', modalBody = $(`#${idName} .${modal.classModalBody}`);
      if (doCustom) {
      ...
      modalBody.append(df);
      $('#pcm-formAddGroupID').keypress( e => { if((e.keyCode ? e.keyCode : e.which) == '13') checkTrigger(doCustom, df, data); });
  ...
  async showDetailsModal(unique, afterClose=null) {
    ...
    modal.showModal(null, async () => {
      let modalBody = $(`#${idName} .${modal.classModalBody}`), detailsDiv = $(`<div id='pcm-modalBody'></div>`).appendTo(modalBody);
      ...
      detailsDiv = null; modalBody = null; detailsTabs = null;
  ...
  editTriggerOptions(editSet, text, text2, validFunc, saveFunc, type, notEmpty=false) {
    ...
    modal.showModal(null, () => {
      let values = Array.from(editSet), df = document.createDocumentFragment(), modalBody = $(`#${idName} .${modal.classModalBody}`);
      ...
      $(`<button class='btn btn-xs pcm-addToSelect'>Add ${text2}</button>`).on( 'click', () => {
        modal.showDialogModal('750px', `Add New ${text}`, `Type in a ${text2}.`, () => {
          let newValue = $('#pcm-formQuestion').val(); $('.pcm-checkStatus.pcm-inputError').html('');
      ...
      $(`<button class='btn btn-xs pcm-removeFromSelect'>Remove Selected</button>`).on( 'click', () => {
        ...
        } else { $('.pcm-checkStatus.pcm-inputError').html('Must have 1 Accepted word or phrase in list!'); }
        removeList = null;
      }).appendTo(df);
      modalBody.append(df);
      $(`#${idName}`).keypress( (e) => { if ((e.keyCode ? e.keyCode : e.which) == '13') saveFunc(); });
      df = null; values = null; form = null; selectBox = null; modalBody = null;
    }, () => { });
  }
  ...
  showTriggerFound(unique, afterClose=null) {
    ...
    modal.showModal(null, async () => {
      ...
      let gidsHistory = await bgHistory.findValues(gidsValues), modalBody = $(`#${idName} .${modal.classModalBody}`);
      ...
      modalBody.append(df);
      groupHist = null; rules = null; blocked = null; df = null; gidsHistory = null; theTable = null; modalBody = null;
    }, () => { modal = null; if (afterClose) afterClose(); });
  }
  ...
  showSearchOptions(afterClose=null) {
    ...
    const idName = modal.prepareModal(theData, '860px', 'pcm-triggerOptModal', 'modal-lg', 'Edit Search General Options', '', '', '', 'visible btn-sm', 'Save Options', (changes) => {
    ...
    modal.showModal(() => {}, async () => {
      ...
      theData = null; modalBody = null; df = null;
    }, () => { modal = null; if (afterClose) afterClose(); });
  }
  ...
  showSearchAdvanced(afterClose=null) {
    if (!modal) modal = new ModalClass();
    const idName = modal.prepareModal(globalOpt.doSearch(), '860px', 'pcm-advancedOptModal', 'modal-lg', 'Edit Search Advanced Options', '', '', '', 'visible btn-sm', 'Save Options', (changes) => {
  ...
  showSearchBlocked(afterClose=null) {
    ...
    modal.showModal(() => {}, async () => {
      let modalBody = $(`#${idName} .${modal.classModalBody}`), blockingDiv = $(`<div id='pcm-blockDetails'></div>`).appendTo(modalBody);
      ...
      modalBody = null; blockingDiv = null;
    }, () => { modal = null; if (afterClose) afterClose(); });
  }
  ...
  showTriggeredHit(theData, afterClose=null, e=null) {
    ...
    modal.showModal(() => {}, async () => {
      let df = document.createDocumentFragment(), blocked = bgSearch.theBlocked(theData.hit_set_id, theData.requester_id), modalBody = $(`#${idName} .${modal.classModalBody}`);
      ...
      $(`<div class='pcm-buttonArea'></div>`).append($(`<button class='btn btn-xs pcm-blockGid'>${(blocked[0]) ? 'UNBLOCK' : 'Block'} this Group ID</button>`).click( e => {
        ...
        $(`<table class='table table-dark table-hover table-sm pcm-detailsTable table-bordered'></table>`).append($(`<tbody></tbody>`).append(df)).appendTo(modalBody);
        df = null; modalBody = null; blocked = null;
    }, () => { modal = null; if (afterClose) afterClose(); });
  }
  ...
  async showTriggersTable(modalBody, triggers, checkboxFunc=null) {
    ...
    for (const dbId of triggers) {
      ...
      if (rules.terms) { data.term = rules.include.values().next().value; }
      else data.term = '';
  ...
  showTriggersModal(type='triggers', groupUnique=-1, thisObj=null, saveFunc=null, checkFunc=null, cancelFunc=null, afterShow=null, afterClose=null) {
    ...
    modal.showModal(cancelFunc, async () => {
      let df = document.createDocumentFragment(), modalControl = $(`<div class='pcm-modal-${type} pcm-modalTriggerControl'></div>`).appendTo(df);
      ...
      $(`<button class='btn btn-xxs pcm-searchingTriggers'>Search</button>`).on( 'click', async () => {
        let theBody = $(`#${idName} .${modal.classModalBody}`); $(theBody).find('.pcm-jobTable').remove();
        let filtered = await this.triggersFilter($('#pcm-searchinput').val().toLowerCase(), $(theBody).find(`pcm-searchInputDiv:first`));
        await this.showTriggersTable(theBody, filtered, checkFunc, () => {}); if (afterShow) afterShow(this);
        theBody = null; filtered = null;
      }).appendTo(inputControl);
      if (type === 'triggers') $(`<button class='btn btn-xxs pcm-deleteSelected'>Delete Selected</button>`).click( () => {
        let theBody = $(`#${idName} .${modal.classModalBody}`), selected = $(theBody).find('.pcm-checkbox:checked').map((_,element) => {
          let trigger = bgSearch.getTrigger($(element).val()); return Number(trigger.count);
        }).get();
        if (selected.length) search.removeJobs(selected, async (response) => {
          if (response !== 'NO') {
            $(theBody).find('.pcm-jobTable').remove();
            let filtered = await this.triggersFilter($('#pcm-searchinput').val().toLowerCase(), $(theBody).find(`pcm-searchInputDiv:first`));
            await this.showTriggersTable(theBody, filtered, checkFunc, () => {}); if (afterShow) afterShow(this); filtered = null;
          }
          theBody = null; selected = null;
        }, () => {}, 'Unselect All');
        else theBody = null;
      }).appendTo(inputControl);
      let modalBody = $(`#${idName} .${modal.classModalBody}`), df2 = document.createDocumentFragment(), filtered = await this.triggersFilter('', modalControl);
      ...
      df = null; df2 = null; modalControl = null; radioGroup = null; inputControl = null; modalBody = null; filtered = null;
