class PandaCard {
  constructor(myId, hitInfo, tabsObj, tabUnique=null) {
    this.myId = myId;
    ...
    this.appendCard(hitInfo, tabUnique);
  ...
  appendCard(hitInfo, tabUnique=null) {
  ...
  showDetailsModal(successFunc=null) {
    function showDetails(hitInfo) {
      const idName = modal.prepareModal(hitInfo.data, "700px", "modal-header-info modal-lg", "Details for a hit", "", "text-right bg-dark text-light", "modal-footer-info", "visible btn-sm", "Save New Details", (changes) => {
        hitInfo.data = Object.assign(hitInfo.data, changes);
        this.updateAllCardInfo(hitInfo);
        bgPanda.updateDbData(this.myId, hitInfo.data);
        modal.closeModal();
    ...
    bgPanda.db.getFromDB(bgPanda.storeName, "get", this.dbId)
      .then( (r) => { bgPanda.info[this.myId].data = r; showDetails.call(this, bgPanda.info[this.myId]);
