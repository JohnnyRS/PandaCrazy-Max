...
class ModalJobClass {
      ...
      if (result) {
        ...
        pandaUI.removeJob(myId, () => {
          ...
          modal.showDialogModal('700px', 'Search trigger created', 'The search job has been moved to the search UI successfully.',_,_, true,_,_,_, (idName) => {
            ...
            let checkboxDiv = $(`<div class='pcm-autoSearchUI small'></div>`).appendTo(modalBody);
  ...
  showJobAddModal(afterClose=null) {
    if (!modal) modal = new ModalClass();
    const idName = modal.prepareModal(null, '920px', 'pcm-addJobsModal', 'modal-lg', 'Add new Panda Info', '<h4>Enter New Panda Information.</h4>', '', '', 'visible btn-sm', 'Add new Panda Info', checkGroupID.bind(this), 'invisible', 'No', null, 'visible btn-sm', 'Cancel');
    let df = document.createDocumentFragment();
    const div = $(`<div><div class='pcm-checkStatus pcm-inputError'></div><div class='pcm-myInfo'>Enter a Group ID, Requester ID, Preview URL or accept URL.</div></div>`).appendTo(df);
    ...
    createInput(df, 'pcm-topBorder', 'pcm-formReqName', 'Requester Name: ', 'default: group ID shown');
    ...
    modal.showModal(null, () => {
      ...
      $('#pcm-formAddGroupID').keypress( (e) => { if((e.keyCode ? e.keyCode : e.which) == '13') checkGroupID.call(this); });
    ...
    function checkGroupID() {
      ...
      if (groupVal === '') {
        ...
        $(div).find('.pcm-checkStatus.pcm-inputError:first').html('Must fill in GroupID or URL!').data('gIdEmpty',true);
        ...
        if (groupId && bgPanda.pandaGroupIds.hasOwnProperty(groupId) && !$(div).find('.pcm-checkStatus.pcm-inputError:first').data('gIdDup')) {
          $(`label[for='pcm-formAddGroupID']`).removeClass('pcm-inputError');
          $(div).find('.pcm-checkStatus.pcm-inputError:first').html('GroupID already added. Still want to add?').data('gIdDup',true);
          ...
          $(`label[for='pcm-formAddGroupID']`).addClass('pcm-inputError');
          $(div).find('.pcm-checkStatus.pcm-inputError:first').html('Invalid Group ID or URL').data('gIdInvalid',true);
        }
      } else {
        $(`label[for='pcm-formAddGroupID']`).addClass('pcm-inputError');
        $(div).find('.pcm-checkStatus.pcm-inputError:first').html('Invalid Group ID or URL').data('gIdInvalid',true);
  ...
  async showJobsTable(modalBody, jobs, type, checkboxFunc=null, afterClose=null) {
    ...
    displayObjectData([
      ...
      {'string':'Requester Name', 'type':'string', 'noBorder':false}, {'string':'Title', 'type':'string', 'noBorder':true}, {'string':'Pay', 'type':'string', 'noBorder':true},
 ...
 showJobsModal(type='jobs', groupUnique=-1, thisObj=null, saveFunc=null, checkFunc=null, cancelFunc=null, afterShow=null, afterClose=null) {
    ...
    const radioGroup = $(`<div class='pcm-uiGroup'></div>`).appendTo(modalControl);
    ...
    $(`<button class='btn btn-xxs pcm-searchingJobs'>Search</button>`).on( 'click', async (e) => {
      $(modalBody).find('.pcm-jobTable').remove();
      let filtered = await this.jobsFilter($('#pcm-searchJobs').val().toLowerCase(), modalControl);
      await this.showJobsTable(modalBody, filtered, type, checkFunc, () => {});
    ...
    if (type === 'jobs') $(`<button class='btn btn-xxs pcm-deleteSelected'>Delete Selected</button>`).click( (e) => {
      ...
      if (selected.length) pandaUI.removeJobs(selected, async (result) => {
          if (result !== 'NO') {
            $(modalBody).find('.pcm-jobTable').remove();
            let filtered = await this.jobsFilter($('#pcm-searchJobs').val().toLowerCase(), modalControl);
            await this.showJobsTable(modalBody, filtered, type,_, () => {});
          }
        }, 'manual', () => {});
    }).appendTo(inputControl);
